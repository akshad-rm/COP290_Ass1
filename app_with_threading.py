...
def update_data():
    nse_data_obj_temp = NSELive()
    company_stocks_temp=[]
    
    for company in company_symbol_map:
        quote = nse_data_obj_temp.stock_quote(company_symbol_map[company])
        temp_dict = {}
        temp_dict["symbol"] = company_symbol_map[company]
        temp_dict["name"] = company
        temp_dict["price"] = quote["priceInfo"]["lastPrice"]
        temp_dict["change"] = quote["priceInfo"]["change"]
        temp_dict["pchange"] = quote["priceInfo"]["pChange"]
        company_stocks_temp.append(temp_dict)
    return company_stocks_temp

def periodic_task():
    while True:
        company_stocks = update_data()
